...
?>
<h1 style="color: #2c3e50; border-bottom: 2px solid #e74c3c; padding-bottom: 10px; margin-bottom: 30px;">QUẢN LÝ DANH
    MỤC</h1>

<a href="?ctrl=admin&act=categoryForm"
    style="display: inline-block; padding: 10px 15px; background: #27ae60; color: white; text-decoration: none; border-radius: 5px; margin-bottom: 20px;">+
    Thêm Danh Mục Mới</a>

<table
    style="width: 100%; border-collapse: collapse; background: #fff; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border-radius: 8px;">
    <thead>
    ...
    <tbody>
        <?php foreach ($categories as $cat): 
            ...
            $buttonClass = $status == 1 ? 'btn-delete' : 'btn-update'; // Tái sử dụng CSS (có thể đổi tên class nếu muốn)
        ?>
        <tr style="border-bottom: 1px solid #eee;">
            ...
            <td style="padding: 15px; text-align: center;">
                <a href="?ctrl=admin&act=categoryForm&id=<?= htmlspecialchars($cat['id'], ENT_QUOTES, 'UTF-8') ?>"
                    style="color: #3498db; text-decoration: none; margin-right: 10px;">Sửa</a>

                <form action="?ctrl=admin&act=categoryToggleStatus" method="POST" style="display: inline;"
                    onsubmit="return confirm('Bạn có chắc chắn muốn chuyển trạng thái danh mục này sang <?= $buttonText ?>?');">
                    ...
                    <button type="submit" class="<?= $buttonClass ?>"
                        style="padding: 5px 10px; border-radius: 5px; font: inherit; cursor: pointer;">
                        <?= $buttonText ?>
                    </button>
                </form>
