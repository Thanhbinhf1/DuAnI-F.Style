        ...
        <div class="left-column" style="flex: 0 0 40%;">
            ...
            <div class="d-flex flex-wrap gap-2">
                ...
                <?php foreach ($galleryImages as $img): 
                    ...
                    ?>
                <img src="<?=$thumb?>" class="thumb-image" data-src="<?=$thumb?>"
                    style="width: 70px; height: 70px; object-fit: cover; border-radius: 5px; cursor: pointer; border: 1px solid #eee;">
                <?php endforeach; ?>
            </div>
            <?php endif; ?>
        ...
        <div class="right-column" style="flex: 1;">
            ...
            <form action="?ctrl=cart&act=add" method="post">
                ...
                <div class="btn-actions">
                    ...
                    <button type="submit" formaction="?ctrl=cart&act=buyNow" class="product-action-btn secondary">
                        Mua ngay
                    </button>
                </div>

            </form>
            <form action="index.php?act=cart-add" method="POST" id="addToCartForm">
                <input type="hidden" name="product_id" value="<?= $product['id'] ?>">

                <div class="mb-3">
                    <label class="fw-bold">Màu sắc:</label>
                    <div class="d-flex gap-2 mt-2">
                        <input type="radio" class="btn-check" name="color" id="color_be" value="Be" autocomplete="off">
                        <label class="btn btn-outline-dark" for="color_be">Be</label>

                        <input type="radio" class="btn-check" name="color" id="color_den" value="Đen"
                            autocomplete="off">
                        <label class="btn btn-outline-dark" for="color_den">Đen</label>

                        <input type="radio" class="btn-check" name="color" id="color_trang" value="Trắng"
                            autocomplete="off">
                        <label class="btn btn-outline-dark" for="color_trang">Trắng</label>
                    </div>
                    <div id="error-color" class="text-danger mt-1" style="display:none; font-size: 14px;">Vui lòng chọn
                        màu sắc!</div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex gap-2 mt-2">
                        ...
                        <label class="btn btn-outline-dark" for="size_s">S</label>

                        <input type="radio" class="btn-check" name="size" id="size_m" value="M" autocomplete="off">
                        <label class="btn btn-outline-dark" for="size_m">M</label>

                        <input type="radio" class="btn-check" name="size" id="size_l" value="L" autocomplete="off">
                        <label class="btn btn-outline-dark" for="size_l">L</label>
                    </div>
                    <div id="error-size" class="text-danger mt-1" style="display:none; font-size: 14px;">Vui lòng chọn
                        kích thước!</div>
                </div>

                <div class="action-buttons mt-4">
                    <button type="submit" class="btn btn-dark w-100 py-2">THÊM VÀO GIỎ HÀNG</button>
                </div>
            </form>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        // Nếu thiếu 1 trong 2 thì chặn form gửi đi
        if (!isValid) {
            e.preventDefault();
            // Có thể thêm alert nếu muốn thông báo bật lên
            // alert("Vui lòng chọn đầy đủ Phân loại hàng (Màu sắc & Size)!");
        }
    });
});
</script>